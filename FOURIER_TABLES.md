...
| 16h | $\dfrac{k\,u(t-a)}{t}$                    | $k\left[-\operatorname{Ci}(a\lvert\omega\rvert) - i\,\operatorname{sgn}(\omega)\left(\dfrac{\pi}{2}-\operatorname{Si}(a\lvert\omega\rvert)\right)\right]$ | $a>0$; vía identidades $\int_a^\infty\!\frac{\cos(\omega t)}{t}dt=-\text{Ci}(a\omega)$, $\int_a^\infty\!\frac{\sin(\omega t)}{t}dt=\frac\pi2-\text{Si}(a\omega)$ ($\omega>0$), extendido por paridad |
| 16i | $\dfrac{k\,u(\lvert t\rvert-a)}{\lvert t\rvert}$ (par, hueco central) | $-2k\,\operatorname{Ci}(a\lvert\omega\rvert)$ | $a>0$; distinto de #16h — el espejo en $t<-a$ cancela la parte impar $\operatorname{Si}/\operatorname{sgn}$, dejando transformada real y par pura; par IFT: $k\,\operatorname{Ci}(a\lvert\omega\rvert)\to -\frac{k}{2}\dfrac{u(\lvert t\rvert-a)}{\lvert t\rvert}$ |
