...
export class CanvasRendererService {
  ...
  private formatConst(x: number, step: number, constVal: number, symbol: string): string {
    if (!Number.isFinite(constVal) || constVal <= 0) {
      return this.formatNumber(x, step);
    }

    const stepRatio = Math.abs(step / constVal);
    const best = this.nearestConstMultiplier(stepRatio);
    const ratioPerTick = best.num / best.den;

    // Tick index n where x = n * ratioPerTick * constVal
    const nRaw = Math.round(x / constVal / ratioPerTick);
    if (nRaw === 0) return '0';

    const numRaw = nRaw * best.num;
    const denRaw = best.den;

    const g = this.gcd(Math.abs(numRaw), denRaw);
    const num = Math.abs(numRaw) / g;
    const den = denRaw / g;
    const sign = numRaw < 0 ? '−' : '';
  ...
  private nearestConstMultiplier(ratio: number): { num: number; den: number } {
    let best = CONST_STEP_MULTIPLIERS[0];
    let bestErr = Math.abs(ratio - best.num / best.den);

    for (const m of CONST_STEP_MULTIPLIERS) {
      const err = Math.abs(ratio - m.num / m.den);
      if (err < bestErr) {
        best = m;
        bestErr = err;
      }
